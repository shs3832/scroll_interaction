(() => {
    ...
    let prevScrollHeight = 0; // 현재 스크롤 위치보다 이전에 위치한 스크롤 섹션 높이합
    ...
    function calcValues(values, currentYOffset) {
        ...
        if (values.length === 3) {
            ...
            if (currentYOffset >= partScrollStart && currentYOffset <= partScrollEnd) {
                ...
                // console.log(`currentYOffset: ${currentYOffset}, partScrollStart: ${partScrollStart}`);
    ...
    function playAnimation() {
        ...
        switch (currentScene) {
            case 0:
                // console.log('0 play');
                // let seq = Math.round(calcValues(values.imageSeq, currentYOffset));
            ...
            case 2:
                // console.log('2 play');
                // let seq2 = Math.round(calcValues(values.imageSeq, currentYOffset));
                ...
                if (scrollRatio > 0.9) {
                    ...
                    const reCalcWidth = document.body.offsetWidth / canvasRatio;
                    const reCalcHeight = window.innerHeight / canvasRatio;
                    const whiteRectWidth = reCalcWidth * 0.15;

                    values.rect1X[0] = (objs.canvas.width - reCalcWidth) / 2;
                    values.rect1X[1] = values.rect1X[0] - whiteRectWidth;
                    values.rect2X[0] = values.rect1X[0] + reCalcWidth - whiteRectWidth;
                    values.rect2X[1] = values.rect2X[0] + whiteRectWidth;

                    objs.context.fillRect(values.rect1X[0], 0, parseInt(whiteRectWidth), reCalcHeight);
                    objs.context.fillRect(values.rect2X[0], 0, parseInt(whiteRectWidth), reCalcHeight);
                }
                break;

            case 3:
                //가로,세로 모두 꽉 차게 하기위한 세팅
                const widthRatio = window.innerWidth / objs.canvas.width;
                const heightRatio = window.innerHeight / objs.canvas.height;
                let step = 0;
                // console.log(widthRatio, heightRatio);
                ...
                if (!values.rectStartY) {
                    ...
                    // console.log(window.innerHeight / 2 / scrollHeight, scrollHeight);
                ...
                objs.context.fillRect(parseInt(calcValues(values.rect1X, currentYOffset)), 0, parseInt(whiteRectWidth), reCalcHeight);
                objs.context.fillRect(parseInt(calcValues(values.rect2X, currentYOffset)), 0, parseInt(whiteRectWidth), reCalcHeight);
                // console.log(whiteRectWidth, whiteRectHeight);
    ...
    window.addEventListener("load", () => {
        ...
        if (tempYOffset > 0) {
            let Interval = setInterval(() => {
                ...
                tempYOffset += tempYOffset;
                tempScrollCount++;
                if (tempScrollCount > 20) clearInterval(Interval);
            }, 20);
        }
        ...
        window.addEventListener("resize", () => {
            ...
            // console.log(sceneInfo[3].values);
        });
